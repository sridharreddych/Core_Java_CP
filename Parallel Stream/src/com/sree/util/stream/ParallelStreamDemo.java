 ...
 * 
 *         Parallel Stream
 *          Collection.parallelStream() returns parallel stream
 *         instance for calling collection. The Stream object can be used for
 *         different purpose. In this example we have taken a list of employee
 *         which has salary field. For a given predicate, we will filter the
 *         list and then will calculate the average salary. Find the code below.
 */
